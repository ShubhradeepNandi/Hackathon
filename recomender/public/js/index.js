...
if(answers.top_class==="temperature")
{

 $.get('/getTemperature').done(function onSucess(data){
   $.post('/getRecommendOnTemperature', data)
      .done(function onSucess(response){            
    var complete = JSON.parse(response, function(k,v){return v});
    $results.show();
    $classification.text(complete.recommendation);
    $('html, body').animate({ scrollTop: $(document).height() }, 'fast');
    $loading.hide();
    });
});
...
if(answers.top_class==="conditions")
{
   $.get('/getTemperature').done(function onSucess(data){
     $.post('/getRecommendOnCondition', data)
        .done(function onSucess(response){            
      var complete = JSON.parse(response, function(k,v){return v});
      $results.show();
      $classification.text(complete.recommendation);
